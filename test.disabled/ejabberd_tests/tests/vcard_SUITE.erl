...
%%                 {<<"BDAY">>, <<"%s">>, [<<"birthDay">>]}, %OpenLDAP doesn't sport it by default
    ...
    ok = escalus_ejabberd:rpc(eldap, modify, [EPid, binary_to_list(Dn), Modificators]);
...
insert_alice_photo(Config) ->
    ...
    {EPid, Base} = get_ldap_pid_and_base(Server),
    Photo = ?PHOTO_BIN,
    Modificators = [escalus_ejabberd:rpc(eldap, mod_replace, ["jpegPhoto", [binary_to_list(Photo)]])],
    Dn = <<"cn=", User/binary, ",", Base/binary>>,
    ok = escalus_ejabberd:rpc(eldap, modify, [EPid, binary_to_list(Dn), Modificators]),
    Config.
...
fields_to_ldap_modificators(VcardMap, [{Field, Val} | Rest], Acc) when is_binary(Val) ->
    case vcard_field_to_ldap(VcardMap, Field) of
        undefined ->
            NewAcc = Acc;
        LdapField ->
            LdapModify = escalus_ejabberd:rpc(eldap, mod_replace, [binary_to_list(LdapField), [binary_to_list(Val)]]),
